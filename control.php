	...
	switch($action){
		case "initconfig":
			if(isset($_POST['dbconfig'])){
				...
				$db = $_POST['dbconfig'];
				$localConfig = str_replace(array('{db-server}','{db-username}','{db-password}','{db-port}','{db-name}',
					'{comment-server}','{comment-username}','{comment-password}','{comment-port}','{comment-name}'
				),array($db['dbserver'],$db['dbusername'],$db['dbpassword'],$db['dbport'],$db['dbname'],
					$db['commentserver'],$db['commentusername'],$db['commentpassword'],$db['commentport'],$db['commentname']
				),$localConfig);
				file_put_contents(DICT_ROOT . '/config/main.php', $localConfig);
